...
function buildRow({ dateStr, week_number, year, source_platform, city, hotel, star_rating }) {
  return {
    ...
    star_rating:         parseInt(star_rating) || null,
    stars:               parseInt(hotel.stars) || null,
    review_score:        parseFloat(hotel.review) || null,
    review_count:        parseInt(hotel.count)   || null,
      ...
      if (hotels.length > 0) {
        ...
        for (const est of HOTELSCOM_ESTIMATES[tier] || []) {
          rows.push({
            ...
            city:                city.name,
            hotel_name:          est.name,
            star_rating:         parseInt(tier) || null,
            stars:               parseInt(tier) || null,
